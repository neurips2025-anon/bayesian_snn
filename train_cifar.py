...
class CustomNet(nn.Module):
        ...
        for i, params in enumerate(conv_params):
            ...
            if scale_style == "learnable":
                ...
                fanin = torch.tensor(conv_layer.weight.shape[1] *
                                     conv_layer.weight.shape[2] *
                                     conv_layer.weight.shape[3], dtype=torch.float32)
                self.scales.append(torch.tensor(torch.log(torch.exp((1)/(fanin**0.5)) - 1).repeat(scale_shape), device=device))


            self.thresholds.append(torch.ones((out_channels, 1, 1), device=device) * 0)
    ...
    def allocate_internal_states(self, b_size, device):
        mem_shapes = []
        for i, params in enumerate(conv_params):
            factor = 1 + (i > 1 * (num_blocks * 2)) + 2 * (i > 2 * (num_blocks * 2))+ 4 * (i > 3 * (num_blocks * 2))+ 8 * (i > 4 * (num_blocks * 2))
    ...
    def forward(self, data, count=1, noise_style="noisy"):
        device = data.device
        b_size = data.size(0)
        ...
        for t in range(time_steps):
            ...
            for j in range(len(self.conv_layers)):
                ...
                if model_style == "bayesian":
                    ...
                    if j == 0:
                        weight =torch.ones((1, data.shape[1], self.conv_layers[j].kernel_size[0], self.conv_layers[j].kernel_size[0]),device=device)
                        added_var_in=(e_sigscale**2)*F.conv2d(data**2, weight, bias=None, stride=self.conv_layers[j].stride, padding=1)

                
                    else:
                        weight =torch.ones((1, self.spike_list[j - 1].shape[1], self.conv_layers[j].kernel_size[0], self.conv_layers[j].kernel_size[0]),device=device)
                    ...
                    total_var_mask = (self.total_var[j] > 0).float()
                    total_var_safe = torch.clamp(self.total_var[j], min=1e-6)
                    ...
                    if j == 0:
                        self.mem_poten_noiseless[0] = beta_mem[0] * self.mem_poten_noiseless[0] + self.bn_layers[time_steps * j + t](self.conv_layers[0](data)) 

                    else:
                        self.mem_poten_noiseless[j] = beta_mem[j] * self.mem_poten_noiseless[j] + self.bn_layers[time_steps * j + t](self.conv_layers[j](self.spike_list[j - 1])) 
...
def train_model(model, train_loader, epochs,device):
    ...
    for epoch in range(epochs):
        ...
        track_layers = n_layers
        batch_count = 0
        relative_sums = [0.0] * track_layers
        ...
        for data, target in train_loader:
            ...
            if model_style == "bayesian":
                for i in range(len(reg_mems)):
                    ...
                    reg_loss += torch.mean(torch.sum(ratio_squared, dim=(1,2,3)))
            ...
            with torch.no_grad():
                for i in range(track_layers):
                    param = model.conv_layers[i].weight
                    g     = param.grad
                    if g is None:
                        continue
        ...
        accuracy, avg_test_loss = evaluate(model, test_loader,device)
        test_losses.append(avg_test_loss)
        accuracies.append(accuracy)

        avg_relative_norms = [s / batch_count for s in relative_sums]
        grad_norm_history.append(avg_relative_norms)


        print(f"Epoch {epoch+1}/{epochs} - Train Loss: {avg_train_loss:.4f} - Train Acc: {avg_train_accuracy:.4f} - Test Loss: {avg_test_loss:.4f} - Acc: {accuracy:.4f}")

    return train_losses, test_losses, accuracies,scale_history, grad_norm_history
...
if __name__ == "__main__":
    ...
    train_losses, test_losses, accuracies,scale_history,grad_norm_history = train_model(model, train_loader, epochs=args.epochs,device=device)

    results[method] = {
        'train_loss': train_losses,
        'test_loss': test_losses,
        'accuracy': accuracies,
        'scale_hist': scale_history,
        'gradnorm_hist': grad_norm_history, 
    }
    ...
    fname = f"{model_style}_cifar.pth"
